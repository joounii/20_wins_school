...
pve_hard () {
    ...
    do
        ...
        if [ $x -ge 20 ]; then
                ...
                jq --arg name "$name" '.users |= map(if .name == $name then .pve_hard[0].lose += 1 else . end)' ./data/user_statistics.json > tmpfile && mv tmpfile ./data/user_statistics.json
                save_statistics
                break
        fi
        ...
        if [ $x -ge 20 ]; then
                ...
                jq --arg name "$name" '.users |= map(if .name == $name then .pve_hard[0].win += 1 else . end)' ./data/user_statistics.json > tmpfile && mv tmpfile ./data/user_statistics.json
                save_statistics
                break
        fi
    done
